...
class M_barang extends CI_Model{
	...
	function tampil_barang(){
		$hsl=$this->db->query("SELECT barang_id,barang_nama,barang_satuan,barang_harpok,barang_harjul,barang_harjul_grosir,barang_stok,barang_min_stok,barang_kategori_id,kategori_nama FROM tbl_barang JOIN tbl_kategori ON barang_kategori_id=kategori_id WHERE kategori_nama<>'Kain'");
		return $hsl;
	}

	function tampil_barang_kain(){
		$hsl=$this->db->query("SELECT barang_id,barang_nama,barang_satuan,barang_harpok,barang_harjul,barang_harjul_grosir,barang_stok,barang_min_stok,barang_kategori_id,kategori_nama FROM tbl_barang JOIN tbl_kategori ON barang_kategori_id=kategori_id WHERE kategori_nama='Kain'");
		return $hsl;
	}

	function tampil_kategori_kain(){
		$hsl=$this->db->query("select * from tbl_kategori WHERE kategori_nama='Kain'");
		return $hsl;
	}

	function tampil_kategori_selain_kain(){
		$hsl=$this->db->query("select * from tbl_kategori WHERE kategori_nama<>'Kain'");
		return $hsl;
	}
